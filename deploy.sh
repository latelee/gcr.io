...
[[ -d "gcr.io" ]] && rm -rf ./gcr.io

# clone repos
git clone "https://github.com/${user_name}/gcr.io.git"
...
function init_imgs()
{
  ...
  next="https://hub.docker.com/v2/repositories/latelee/${n}.${img}/tags/?page_size=100"
  while [ "null" != "${next}" ];do
...
function compare()
{
  ...
  find ./gcr.io/ -name "*.tmp" | while read t
  do
    dir=$(dirname $t)
    name=$(basename $t .tmp)
...
function pull_push_diff()
{
  n=$1
  img=$2
  all_of_imgs=$(find ./gcr.io -type f -name "*.t*" |wc -l)
  current_ns_imgs=$(find ./gcr.io/${n}/ -type f -name "*.t*" |wc -l)
  tmps=($(find ./gcr.io/${n}/${img}/ -type f \( -iname "*.tmp" \) -exec basename {} .tmp \; | uniq))
  ...
  for tag in ${tmps[@]} ; do
    ...
    mv ./gcr.io/${n}/${img}/${tag}.tmp ./gcr.io/${n}/${img}/${tag}.tag
...
function mirror()
{
  num=$(find ./gcr.io/ -type f \( -iname "*.tmp" \) |wc -l)
  if [ $num -eq 0 ]; then
  ...
  sleep 30
  compare
  find ./gcr.io/ -type f -name "*.t" -exec rm -rf {} \;
  
  tmp_imgs=$(find ./gcr.io/ -type f \( -iname "*.tmp" \) -exec dirname {} \; | uniq | cut -d'/' -f3-4)
  ...
  images=($(find ./gcr.io/ -type f -name "*.tag" |uniq|sort))
  
  cp ./gcr.io/CHANGES.md ./CHANGES1.md 2>/dev/null
  
  [[ ! -s ./CHANGES1.md ]] && touch ./CHANGES1.md
  
  find ./gcr.io/ -type f -name "*.md" -exec rm -rf {} \;
    
  for img in ${images[@]} ; do
    n=$(echo ${img}|cut -d'/' -f3)
    image=$(echo ${img}|cut -d'/' -f4)
    tag=$(basename ${img} .tag)
    mkdir -p ./gcr.io/${n}/${image}
    if [ ! -e ./gcr.io/${n}/${image}/README.md ]; then
      echo -e "\n[gcr.io/${n}/${image}](https://hub.docker.com/r/${user_name}/${n}.${image}/tags/)\n-----\n\n" >> ./gcr.io/${n}/${image}/README.md
      echo -e "\n[gcr.io/${n}/${image}](https://hub.docker.com/r/${user_name}/${n}.${image}/tags/)\n-----\n\n" >> ./gcr.io/${n}/README.md
    fi
  ...
  if [ -s CHANGES.md ]; then
    (echo -e "## $(date +'%Y-%m-%d %H:%M') \n" && cat CHANGES.md 2>/dev/null&& cat CHANGES1.md 2>/dev/null) >> gcr.io/CHANGES.md
  fi
  commit
}
...
function commit()
{
  echo 1 > ./commit.lck
  ns=($(cat ./gcr_namespaces 2>/dev/null || echo google-containers))
  readme=./gcr.io/README.md
  export current_date=$(date +'%Y-%m-%d %H:%M')
  cat README.md | envsubst '$user_name $current_date' > "${readme}"
  ...
  git -C ./gcr.io pull
  git -C ./gcr.io add -A
  git -C ./gcr.io commit -m "sync gcr.io's images at $(date +'%Y-%m-%d %H:%M')"
  git -C ./gcr.io push -f "https://${user_name}:${GH_TOKEN}@github.com/${user_name}/gcr.io_mirror.git" master:master
